...
class SantaBot:
    ...
    def __init__(self, dbConnection):
        ...
        self.handlers = [
            ...
            CommandHandler('profile', self.show_address),
    ...
    def start(self, update: Update, context: CallbackContext):
        try:
            ...
            if chat_type != "private":
                message = _('Вы должны отправить мне это в приватном чате')
            ...
            if this_participant is None:
                ...
                self.session.commit()
                message = _('Здравствуйте! Давайте приступим к настройке!')
                self.send_message(
                    context=context,
                    chat_id=this_participant.telegram_id,
                    text=message,
                )
                message = _('Как твое имя и что ты хочешь? (Ответьте на это сообщение, чтобы изменить его)')  # noqa: E501
                self.send_message(
                    context=context,
                    chat_id=this_participant.telegram_id,
                    text=message,
                ...
                already_setup = _('Вы уже настроены.Ваш профиль:') + '\n'  # noqa: E501
    ...
    def help(self, update: Update, context: CallbackContext):
        ...
        message = (
            ...
            "/start \n" +
            _("Отправляется только в личном сообщении, чтобы начать личную настройку.") + "\n" +  # noqa: E501

            "/hello \n" +
            _("Отправляется только в групповом чате для возможности обмена подарками.") + "\n" +  # noqa: E501

            "/profile \n" +
            _("Отправляется только в личном сообщении для отображения и обновления вашего профиля.") + "\n" +  # noqa: E501

            "/join \n" +
            _("Присоединяется к вам в обмене подарками в этом чате.") + "\n" +  # noqa: E501

            "/status \n" +
            _("Получает статус обмена в этом чате.") + "\n" +

            "/not @Mention \n" +
            _("Предотвращает создание пары с этим участником.") + "\n" +  # noqa: E501

            "/allow @Mention \n" +
            _("Удаляет блокировку, которая мешала вам соединиться с этим участником.") + "\n" +  # noqa: E501

            "/leave \n" +
            _("Вы оставите обмен подарками в этом чате.") + "\n" +

            "/start_exchange \n" +
            _("Начинает обмен подарками, назначая получателя каждому участнику, а затем сообщая им детали в частном порядке.") +

            "/reset_exchange \n" +
            _("Сбрасывает обмен подарками, удаляя всех назначенных получателей.")  # noqa: E501
        )
        self.reply_message(update=update, text=message)
    ...
    def show_address(self, update: Update, context: CallbackContext):
        try:
            ...
            this_participant = self.session.query(Participant).filter(
                Participant.telegram_id == user_id).first()
            if this_participant is None:
                message = _("Отправьте мне {command} в личном сообщении и следуйте инструкциям!") \
                    .format(command='/start')  # noqa: E501
                self.reply_message(update=update, text=message)
                return
            else:
                message = (
                    _("В настоящее время у меня есть ваш профиль {address}")
                    .format(address=str(this_participant.address)) + "\n" +
                    _("Если это верно, игнорируйте следующее сообщение.")  # noqa: E501
                )
                self.send_message(
                    context=context,
                    chat_id=this_participant.telegram_id,
                ...
                message = _("Как твое имя, класс и что ты хочешь? (Ответьте на это сообщение, чтобы изменить его)")  # noqa: E501
                self.send_message(
                    context=context,
                    chat_id=this_participant.telegram_id,
                    text=message,
    ...
    def address(self, update: Update, context: CallbackContext):
        try:
            ...
            if (
                ...
                original_text == _("Как твое имя, класс и что ты хочешь? (Ответьте на это сообщение, чтобы изменить его)")  # noqa: E501
            ):
                this_user = (
                    self.session
                    .query(Participant)
                ...
                message = (
                    _("Я добавил ваш профиль как: ") +
                    new_address + "\n" +
                    _("Теперь вы можете использовать {command} в любой группе Telegram Секретный санта!")  # noqa: E501
                    .format(command='/join') + "\n" +
                    _("Группу Telegram Секретный санта нужно активировать только один раз.") + "\n" +  # noqa: E501
                    _("Для этого я должен быть членом группы телеграмм, и кто-то должен активировать меня с помощью команды {command}")  # noqa: E501
    ...
    def hello(self, update: Update, context: CallbackContext):
        try:
            ...
            chat_type = update.effective_chat.type
            if chat_type == "private":
                message = _("Вы должны начать с группового чата")
                self.reply_message(update=update, text=message)
                return
            ...
            message = (
                _("Здравствуйте! ") +
                _("В этом групповом чате теперь есть возможность участвовать в обмене секретным Санта-Клаусом!") + "\n" +  # noqa: E501
                _("{command} для участия").format(command='/join')
            )
            self.reply_message(update=update, text=message)
            return
        except Exception as this_ex:
    ...
    def join(self, update: Update, context: CallbackContext):
        try:
            ...
            if this_participant.address is None:
                message = _("Сначала мне нужно настроить ваш профиль! Я отправляю вам личное сообщение.")  # noqa: E501
                self.reply_message(update=update, text=message)
                message = _("Как твое имя, класс и что ты хочешь? (Ответьте на это сообщение, чтобы изменить его)")  # noqa: E501
                self.send_message(
                    context=context,
            ...
            if this_link is None:
                ...
                if this_group is None:
                    message = _("Кто-то должен сначала сказать {command}!") \
                        .format(command='/hello')
                    self.reply_message(update=update, text=message)
                    return
                self.session.add(
                ...
                message = _("Вы уже присоединились!")
                self.reply_message(update=update, text=message)
        except Exception as this_ex:
            logging.exception(this_ex)
                ...
                if entity_type == "mention":
                    ...
                    if participant_by_username is None:
                        ...
                        if already_blocked is None:
                            ...
                            message = _("Это заблокированное сопряжение уже добавлено.")  # noqa: E501
                            self.reply_message(update=update, text=message)
                    elif participant_by_username.id == this_participant.id:
                        message = _("Не волнуйся, ты не получишь себя")
                        self.reply_message(update=update, text=message)
                    else:
                    ...
                    if mentioned_participant is None:
                        message = _("Этот участник должен сначала присоединиться")
                        self.reply_message(update=update, text=message)
                    else:
                        id_list = [this_participant.id,
                        ...
                        if in_blocked_list is None:
                            ...
                            message = _("Вы больше не можете соответствовать этому участнику.")  # noqa: E501
                            self.reply_message(update=update, text=message)
                        else:
                            message = _("Это заблокированное сопряжение уже добавлено.")  # noqa: E501
                            self.reply_message(update=update, text=message)
            ...
            for entity, entity_text in entities.items():
                ...
                if entity_type == "mention":
                    ...
                    if blocked_link is not None:
                        ...
                        self.session.commit()
                        message = _("Теперь вас могут назначить на {name}") \
                            .format(name=entity_text)
                        self.reply_message(update=update, text=message)
                    else:
                        message = _("{name} не был заблокирован вами.") \
                            .format(name=entity_text)
                        self.reply_message(update=update, text=message)
                elif entity_type == "text_mention":
                    this_participant = (
    ...
    def leave(self, update: Update, context: CallbackContext):
        try:
            ...
            if this_link is None:
                message = _("Вы никогда не присоединялись")
                self.reply_message(update=update, text=message)
            else:
                self.session.query()
    ...
    def status(self, update: Update, context: CallbackContext):
        try:
            ...
            message += _("Присоединенные пользователи:")
            for link in group_links:
                ...
                if blocked_participants:
                    for blocked_participant in blocked_participants:
                        if first:
                            message += _("Невозможно соединиться с: ")
                            first = False
                        else:
                            message += ', '
    ...
    def start_exchange(self, update: Update, context: CallbackContext):
        try:
            ...
            if link_record_to_check is None:
                message = _("Никто не присоединился!")
                self.reply_message(update=update, text=message)
                return
            if link_record_to_check.receiver_id is not None:
                message = _("Обмен уже настроен!")
                self.reply_message(update=update, text=message)
                return
            ...
            if not success:
                message = _("Совпадение невозможно: 0 перестановок")
                self.reply_message(update=update, text=message)
                return
    ...
    def save_and_send_exchange_results(
        ...
        for santa_id, receiver_id in selected_combination.items():
            ...
            if not success:
                ...
                return
        self.session.commit()
        message = _("Сообщения отправлены!")
        self.reply_message(update=update, text=message)
    ...
    def set_and_send_individual_result(
        ...
        try:
            ...
            message = _("Ошибка: {error}. Идентификатор ошибки: {id} отправте ошибку мне в лс инстаграмма(0-0)") \
    ...
    def reset_exchange(self, update: Update, context: CallbackContext):
        ...
        self.session.commit()
        message = _("Все пары были сброшены")
        self.reply_message(update=update, text=message)
